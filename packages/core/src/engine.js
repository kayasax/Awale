...
function playerPitRange(player) {
...
export function getLegalMoves(state) {
    ...
    const opponent = state.currentPlayer === 'A' ? 'B' : 'A';
    const [oStart, oEnd] = playerPitRange(opponent);
    const opponentHasSeeds = state.pits.slice(oStart, oEnd + 1).some((v) => v > 0);
    if (opponentHasSeeds)
        return candidates; // normal case
...
function feedsOpponent(state, pitIndex) {
...
function belongsToOpponent(current, pitIndex) {
...
export function applyMove(state, pitIndex) {
    ...
    const newState = { ...state, pits: [...state.pits], captured: { ...state.captured }, turn: state.turn + 1, lastMove: pitIndex, version: (state.version || 0) + 1 };
    ...
    while (seeds > 0) {
        idx = (idx + 1) % 12;
        if (idx === pitIndex)
            continue; // skip original pit per standard Oware rule variant
        newState.pits[idx] += 1;
        seeds--;
    }
    ...
    const remainingSeeds = newState.pits.reduce((a, b) => a + b, 0);
...
function distributeRemaining(state) {
    if (state.ended)
        return;
    state.captured.A += state.pits.slice(0, 6).reduce((a, b) => a + b, 0);
    state.captured.B += state.pits.slice(6, 12).reduce((a, b) => a + b, 0);
    for (let i = 0; i < 12; i++)
        state.pits[i] = 0;
}
function finalizeWinner(state) {
    state.ended = true;
    if (state.captured.A > state.captured.B)
        state.winner = 'A';
    else if (state.captured.B > state.captured.A)
        state.winner = 'B';
    else
        state.winner = 'Draw';
}
export function formatBoard(state) {
    const top = state.pits.slice(6, 12).slice().reverse().map((n) => n.toString().padStart(2, ' ')).join(' ');
    const bottom = state.pits.slice(0, 6).map((n) => n.toString().padStart(2, ' ')).join(' ');
